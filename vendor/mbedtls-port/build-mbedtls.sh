...
CFLAGS=(-mcpu=cortex-m7 -mthumb -mfloat-abi=hard -mfpu=fpv5-d16 -Os -g -ffreestanding -ffunction-sections -fdata-sections
  ...
  -I"$HERE" -I"$ROOT/vendor/mbedtls/include" -I"$ROOT/vendor/mbedtls/library" -I"$LIB/include")
n=0
...
if [ $LINKPROOF = 1 ]; then
  ...
  for c in "$HERE/linkproof.c" "$ROOT/boards/startup.c" "$ROOT/boards/clock.c"; do $CC "${CFLAGS[@]}" -DAMC_BOARD_HEADER='"Amalgame_Mcu_Board.h"' -I"$ROOT/runtime" -c "$c" -o "$O/$(basename "$c").o"; done
